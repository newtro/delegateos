...
import { createCompletionAttestation, verifyAttestationSignature } from '../core/attestation.js';
import type {
  Keypair,
  SerializedDCT,
  Attestation,
...
export class SpecialistAgent {
  ...
  async review(
    ...
    for (const file of task.files) {
      const hasAccess = scope.capabilities.some(cap =>
        matchesGlob(file, cap.resource)
      );
      if (!hasAccess) {
  ...
  private generateFindings(files: string[]): Finding[] {
