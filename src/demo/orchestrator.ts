...
import { createDCT, attenuateDCT, inspectDCT } from '../core/dct.js';
...
import { SpecialistAgent, type ReviewTask, type ReviewResult, type Finding } from './specialist.js';
...
export class OrchestratorAgent {
  ...
  planDelegations(
    ...
    for (const [specialty, files] of Object.entries(filesBySpecialty)) {
      ...
      if (!specialist || files.length === 0) continue;
      ...
      const allowedCapabilities: Capability[] = resourcePatterns.flatMap(pattern => [
  ...
  async executeReview(
    request: PRReviewRequest,
    contractId: string,
    revokeSpecialty?: string, // Optional: revoke this specialty mid-flow for demo
    ...
    const results: MergedReview['specialistResults'] = [];
    const attestationChain: string[] = [];
    ...
    for (const plan of plans) {
      ...
      try {
        ...
        const attestationValid = verifyAttestationSignature(
          reviewResult.attestation,
          plan.specialist.keypair.principal.id,
        );
    ...
    const recommendation: MergedReview['overallRecommendation'] =
      criticalCount > 0 ? 'request_changes' :
  ...
  private getResourcePatterns(specialty: string): string[] {
    switch (specialty) {
