 ...
 */
export function decompose(contract: TaskContract, strategy: DecompositionStrategy): DecompositionPlan {
...
export function validatePlan(plan: DecompositionPlan, parentContract: TaskContract): Result<void> {
  ...
  if (totalBudget > parentContract.constraints.maxBudgetMicrocents) {
    return {
      ok: false,
      error: new Error(
        `Total sub-task budget (${totalBudget}) exceeds parent budget (${parentContract.constraints.maxBudgetMicrocents})`
      ),
    };
  }
  ...
  for (const task of plan.subTasks) {
    if (task.deadline > parentContract.constraints.deadline) {
      return {
        ok: false,
        error: new Error(
          `Sub-task "${task.title}" deadline (${task.deadline}) exceeds parent deadline (${parentContract.constraints.deadline})`
        ),
      };
    }
  }
    ...
    for (const cap of task.capabilities) {
      if (!parentCaps.has(cap.namespace) && !parentCaps.has('*')) {
        return {
          ok: false,
          error: new Error(
            `Sub-task "${task.title}" capability namespace "${cap.namespace}" not in parent required capabilities`
          ),
        };
      }
    }
...
function detectCycle(tasks: SubTask[]): string | null {
 ...
 */
export class SequentialStrategy implements DecompositionStrategy {
  name = 'sequential';
 ...
 */
export class ParallelStrategy implements DecompositionStrategy {
  name = 'parallel';
