...
export function verifyDCT(
  ...
  const matched = effectiveCaps.some(cap =>
    matchCapability(cap, requestedNamespace, context.operation, context.resource)
  );
  if (!matched) {
    return {
      ok: false,
 ...
 */
export function inspectDCT(serialized: SerializedDCT): {
  issuer: string;
  delegatee: string;
  contractId: string;
  delegationId: string;
 ...
 */
export function getRevocationIds(serialized: SerializedDCT): string[] {
  const token = deserialize(serialized);
  const ids: string[] = [computeRevocationId(token.authority)];
  for (const att of token.attenuations) {
...
function serialize(token: SignedJSONToken): SerializedDCT {
  const json = canonicalize(token);
  const bytes = new TextEncoder().encode(json);
  return { token: toBase64url(bytes), format: 'delegateos-sjt-v1' };
}

function deserialize(serialized: SerializedDCT): SignedJSONToken {
  ...
  return JSON.parse(json) as SignedJSONToken;
}

function computeRevocationId(block: unknown): string {
  const payload = new TextEncoder().encode(canonicalize(block));
  return toBase64url(blake2b256(payload));
}

function computeEffective(token: SignedJSONToken): {
  capabilities: Capability[];
  budget: number;
  expiresAt: string;
  ...
  for (const att of token.attenuations) {
    if (att.allowedCapabilities) capabilities = att.allowedCapabilities;
    if (att.maxBudgetMicrocents !== undefined) budget = att.maxBudgetMicrocents;
    if (att.expiresAt !== undefined) expiresAt = att.expiresAt;
    if (att.maxChainDepth !== undefined) maxChainDepth = att.maxChainDepth;
  }
...
function isCapabilitySubset(cap: Capability, parentCaps: Capability[]): boolean {
  return parentCaps.some(parent =>
    parent.namespace === cap.namespace &&
...
function isResourceSubset(child: string, parent: string): boolean {
  ...
  if (parent.endsWith('/**')) {
    ...
    if (child.startsWith(prefix)) return true;
  }
  // Parent ends with /* and child is one level deeper
  if (parent.endsWith('/*')) {
    const prefix = parent.slice(0, -2);
    if (child.startsWith(prefix) && !child.slice(prefix.length).includes('/')) return true;
  }
  return false;
}

/** Check if a capability matches a namespace+operation+resource request */
function matchCapability(cap: Capability, namespace: string, operation: string, resource: string): boolean {
  if (namespace && cap.namespace !== namespace && cap.namespace !== '*') return false;
  if (cap.action !== operation && cap.action !== '*') return false;
  ...
  while (pi < patParts.length && vi < valParts.length) {
    if (patParts[pi] === '**') {
      ...
      for (let k = vi; k <= valParts.length; k++) {
        if (matchGlob(patParts.slice(pi + 1).join('/'), valParts.slice(k).join('/'))) return true;
      }
      return false;
    }
