...
export class AgentRegistry {
    ...
    discover(filter) {
        ...
        if (filter.namespaces && filter.namespaces.length > 0) {
            results = results.filter(card => filter.namespaces.some(ns => card.delegationPolicy.allowedNamespaces.includes(ns) ||
                card.capabilities.some(c => c.namespace === ns)));
        }
        if (filter.capabilities && filter.capabilities.length > 0) {
            results = results.filter(card => filter.capabilities.every(reqCap => card.capabilities.some(agentCap => agentCap.namespace === reqCap.namespace &&
