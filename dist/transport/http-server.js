...
export class MCPHttpServer {
    ...
    metrics;
    rateLimiter;
    constructor(config, mcpPlugin, registry, opts) {
        this.config = config;
    ...
    async handleMessage(req, res) {
        ...
        const resp = {
            id: msg.id,
            result: { method: result.method, params: result.params },
        };
        this.sendJson(res, 200, resp);
    }
     ...
     */
    pushEvent(sessionId, event, data) {
        const session = this.sessions.get(sessionId);
        if (!session || !session.writer || session.writer.isClosed)
            return false;
    ...
    toMCPRequest(msg, dct) {
        ...
        if (dct) {
            mcpReq.params._delegateos = {
                ...
                delegationId: params._delegateos?.delegationId ?? 'unknown',
                contractId: params._delegateos?.contractId ?? 'unknown',
            };
        }
        return mcpReq;
    }
