...
export function createMCPPlugin(config) {
    const audit = new AuditLog();
    const logger = createLogger('MCPPlugin');
    let _lastAuthorizedMeta = null;
    ...
    async function handleRequest(req) {
        ...
        for (const root of config.trustedRoots) {
            ...
            if (result.ok) {
                authorized = true;
                globalMetrics.counter('mcp.authorized', { tool: toolName });
        ...
        // All roots failed
        globalMetrics.counter('mcp.denied', { tool: toolName });
