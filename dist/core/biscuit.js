 ...
 */
import { toBase64url, fromBase64url, canonicalize, signObject, verifyObjectSignature, } from './crypto.js';
...
export class DatalogEvaluator {
    ...
    hasFact(fact) {
        return this.facts.some(f => f.name === fact.name && f.terms.length === fact.terms.length &&
     ...
     */
    matchBody(body, index, bindings) {
        if (index >= body.length)
            return [new Map(bindings)];
        const atom = body[index];
        const results = [];
        for (const fact of this.facts) {
            if (fact.name !== atom.name || fact.terms.length !== atom.terms.length)
                continue;
...
function evalConstraint(val, op, ref) {
 ...
 */
export function createBiscuitDCT(params) {
    const facts = [];
 ...
 */
export function attenuateBiscuitDCT(params) {
 ...
 */
export function verifyBiscuitDCT(serialized, context) {
    let token;
    try {
    ...
    for (const block of token.blocks) {
        ...
        if (blockCaps.length > 0)
            capabilities = blockCaps;
        const blockBudget = getFactValue(block.facts, 'max_budget');
        if (blockBudget !== null)
            maxBudget = Number(blockBudget);
    ...
    const matched = capabilities.some(cap => {
        if (requestedNamespace && cap.namespace !== requestedNamespace && cap.namespace !== '*')
            return false;
        if (cap.action !== context.operation && cap.action !== '*')
            return false;
