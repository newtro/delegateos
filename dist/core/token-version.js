...
export const TOKEN_CURRENT_VERSION = { major: 1, minor: 0 };
// ── Version Helpers ──
 ...
 */
export function isCompatible(tokenVersion, engineVersion) {
...
export function setTokenVersion(token, version) {
    try {
        const json = JSON.parse(new TextDecoder().decode(fromBase64url(token.token)));
 ...
 */
export function migrateToken(token, fromVersion, toVersion) {
    ...
    while (current.major !== toVersion.major || current.minor !== toVersion.minor) {
        const migration = migrations.find(m => m.from.major === current.major && m.from.minor === current.minor
