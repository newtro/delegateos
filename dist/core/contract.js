...
export class CheckFunctionRegistry {
    ...
    get(name) {
        ...
        if (!fn)
            throw new Error(`Check function not found: ${name}`);
        return fn;
    }
...
function getField(output, field) {
    if (!field)
        return output;
 ...
 */
export function createContract(issuer, task, verification, constraints) {
    const contract = {
 ...
 */
export function verifyContractSignature(contract, issuerPublicKey) {
 ...
 */
export function verifyOutput(contract, output, registry) {
    try {
...
function runVerification(spec, output, registry) {
    switch (spec.method) {
        case 'schema_match': {
            if (!spec.schema)
                throw new Error('schema_match requires schema');
        ...
        case 'deterministic_check': {
            if (!spec.checkName)
                throw new Error('deterministic_check requires checkName');
        ...
        case 'composite': {
            if (!spec.steps || !spec.mode)
                throw new Error('composite requires steps and mode');
            const results = [];
